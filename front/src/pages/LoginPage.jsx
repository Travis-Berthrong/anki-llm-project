...
export function LoginPage() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios_instance.post(requests.login, { username, password });
      console.log(response.data);
    } catch (error) {
      if (error?.response?.status === 400) {
        setAlert({title: 'Invalid username or password', description: 'Please try again.', variant: 'destructive'});
      } else {
